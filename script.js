...
function initPageSpecifics(namespace) {
  ...
  if (namespace === 'home') {
    // Typewriter plays first; the spotlight (hover-gold) effect starts
    // only after typing finishes so the two never conflict.
...
═══════════════════════════════════════════════ */
/* Types out "Where culture meets the plate" character by character with a
   blinking gold cursor. Plays once after the loader / Barba transition
   finishes (initPageSpecifics is called at that point). The spotlight
   (hover-gold) effect starts via onComplete when typing finishes so the
   two never visually conflict. */
function initHeadlineTypewriter(onComplete) {
  ...
  const visibleText = 'Where culture meets the plate';
  const srOnlyText = ' \u2014 Food Photography by Dijo Studios, Johannesburg';

  if (_prefersReducedMotion) {
    // Show the full text immediately (already in the HTML, but ensure it).
    base.innerHTML = visibleText + '<span class="sr-only">' + srOnlyText + '</span>';
    if (onComplete) onComplete();
    return;
  }

  // Build the typewriter DOM:
  //   <span class="sr-only">full accessible text</span>
  //   <text-node></text-node>           ← grows as we type
  //   <span class="tw-cursor"></span>    ← blinking gold bar
  const srOnly = document.createElement('span');
  srOnly.className = 'sr-only';
  srOnly.textContent = visibleText + srOnlyText;
  ...
  base.innerHTML = '';
  base.appendChild(srOnly);
  base.appendChild(textNode);
  base.appendChild(cursor);

  let i = 0;
  const speed = 65; // ms per character — deliberate, premium feel

  const type = () => {
    if (i < visibleText.length) {
      textNode.data = visibleText.slice(0, ++i);
      setTimeout(type, speed);
    } else {
      // Typing done — let the cursor blink a few times, then remove it
      // and start the spotlight hover effect.
      setTimeout(() => {
  ...
  // Small delay so the eye settles after the loader/transition reveals the page.
  setTimeout(type, 250);
}
