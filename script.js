...
function initScrollIndicator() {
  ...
  // Fade the indicator out as the user scrolls past the hero (1vh past hero bottom).
  // Listen to whichever scroll system is active (Lenis on desktop, native on mobile).
  let _indicatorRaf = false;
  const updateIndicatorVisibility = () => {
    if (_indicatorRaf) return;
    _indicatorRaf = true;
    requestAnimationFrame(() => {
      const heroBottom = indicator.parentElement.getBoundingClientRect().bottom;
      // If hero's bottom edge is above 80% of viewport, user has scrolled enough — fade out.
      const shouldHide = heroBottom < window.innerHeight * 0.6;
