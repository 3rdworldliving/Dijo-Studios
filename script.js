...
function initPageSpecifics(namespace) {
  ...
  if (namespace === 'home') {
    // Typewriter plays first (caret-only pause, then types), then the
...
/* ═══════════════════════════════════════════════
   7. HEADLINE TYPEWRITER
═══════════════════════════════════════════════ */
/* After the loader/transition reveals the page, the hero is blank except a
   blinking yellow caret for ~500ms, then three lines type out in sequence:
     1. "Food Photographer · Johannesburg"  (subtitle)
     2. "Where culture meets the plate"     (headline)
     3. "Capturing the stories…"             (paragraph)
   A single caret moves between the three elements as each types. Once the
   last character lands, the caret is removed and the spotlight (hover-gold)
   effect starts. */
function initHeadlineTypewriter(onComplete) {
  const base = document.getElementById('headline-base');
  if (!base) { if (onComplete) onComplete(); return; }

  const hero = document.querySelector('.hero-content');
  if (!hero) { if (onComplete) onComplete(); return; }

  const subtitleEl = hero.querySelector('.subtitle');
  const paraEl = hero.querySelector('p:last-of-type');

  const subtitleText = subtitleEl ? subtitleEl.textContent.trim() : '';
  const headlineVisible = 'Where culture meets the plate';
  const headlineSrOnly  = ' \u2014 Food Photography by Dijo Studios, Johannesburg';
  const paraText = paraEl ? paraEl.textContent.trim() : '';

  // Ordered list of elements to type.
  const targets = [];
  if (subtitleEl && subtitleText) targets.push({ el: subtitleEl, text: subtitleText });
  targets.push({ el: base, text: headlineVisible, srOnly: headlineSrOnly });
  if (paraEl && paraText) targets.push({ el: paraEl, text: paraText });

  if (_prefersReducedMotion) {
    // Show everything immediately.
    if (subtitleEl) subtitleEl.textContent = subtitleText;
    base.innerHTML = headlineVisible + '<span class="sr-only">' + headlineSrOnly + '</span>';
    if (paraEl) paraEl.textContent = paraText;
    if (onComplete) onComplete();
    return;
  }

  // Set up each element: sr-only full text (for screen readers) + empty
  // text node (will grow as we type). A single caret moves between elements.
  ...
  targets.forEach(t => {
    ...
    sr.textContent = t.srOnly ? (t.text + t.srOnly) : t.text;
    t.el.appendChild(sr);
    t.textNode = document.createTextNode('');
    t.el.appendChild(t.textNode);
  });

  // Reveal the hero — the original text is now cleared, so only the caret
  // (in the first element) is visible. The page looks blank with a caret.
  hero.classList.remove('hero-pending');

  // Place the caret in the first element.
  targets[0].el.appendChild(cursor);

  let targetIdx = 0;
  let charIdx = 0;
  // Per-element typing speed (ms per character). The headline is the star of
  // the show, so it types noticeably slower than the subtitle/paragraph for a
  // more deliberate, dramatic reveal.
  const speeds = [28, 70, 28];
  const caretDelay = 500;     // blank page + caret only for half a second
  const pauseBetween = 200;   // ms pause between elements

  const typeNext = () => {
    const t = targets[targetIdx];
    if (charIdx < t.text.length) {
      charIdx++;
      t.textNode.data = t.text.slice(0, charIdx);
      setTimeout(typeNext, speeds[targetIdx] || 45);
    } else {
      // Current element done.
      targetIdx++;
      if (targetIdx < targets.length) {
        // Move caret to next element, brief pause, then continue typing.
        if (cursor.parentNode) cursor.parentNode.removeChild(cursor);
        charIdx = 0;
        targets[targetIdx].el.appendChild(cursor);
        setTimeout(typeNext, pauseBetween);
      } else {
        // All three elements typed — caret stays blinking at the end of the
        // last element (the paragraph). Spotlight effect starts.
        if (onComplete) onComplete();
      }
    }
  };

  // Caret-only pause, then start typing the first element.
  setTimeout(typeNext, caretDelay);
}

/* ═══════════════════════════════════════════════
   7b. SPOTLIGHT HEADLINE
═══════════════════════════════════════════════ */
...
function initAnimations() {
  ...
  if (_isTouchDevice) {
    document.querySelectorAll('h1, h2').forEach(heading => {
      ...
      if (heading.id === 'headline-base' || heading.id === 'headline-highlight') return; // handled by typewriter
      gsap.fromTo(heading,
        { opacity: 0, y: 30 },
    ...
    document.querySelectorAll('h1, h2').forEach(heading => {
      ...
      ScrollTrigger.create({
        ...
        onEnter: () => {
          ...
          gsap.to(split.chars, { color: '#ffc62d', duration: 0.35, stagger: { each: 0.03, yoyo: true, repeat: 1 }, delay: 0.2 });
        }
      });
    });
...
/* ═══════════════════════════════════════════════
   10. READING-ORDER SORT + LIGHTBOX ORDER
═══════════════════════════════════════════════ */
let snakeOrderedCards = [];
let snakeOrderedSrcs = [];

/* Sort gallery cards into standard reading order: row by row, each row
   left-to-right. (Previously a "snake"/boustrophedon order that reversed
   alternate rows — now pure left-to-right so the lightbox cycles through
   images the way you'd read them.) */
...
/* Recompute the lightbox order from the current (settled) DOM positions.
   Called when the user opens the lightbox so the cycling order always
   matches the true visual left-to-right reading order. */
function refreshLightboxOrder() {
  const allCards = [...document.querySelectorAll('.gallery-card, .featured-card')];
  const ordered = computeSnakeOrder(allCards);
...
function makeCard(src, globalIndex) {
  ...
  const openFromCard = () => {
    if (card.classList.contains('img-failed')) return;
    // Recompute the lightbox order from current DOM positions — by the time
    // the user clicks, the masonry layout has settled (images loaded, spans
    // set), so this gives the true visual left-to-right reading order. The
    // order computed during snakeReveal (scroll-trigger) may have run before
    // the layout was settled and can be stale.
    refreshLightboxOrder();
    const idx = snakeOrderedCards.indexOf(card);
