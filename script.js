...
function initAnimations() {
  ...
  /* ── A. SplitType: word-by-word heading reveals ──
     Splits every h1 and h2 into individual word spans,
     then GSAP stagger-animates them up into view on scroll */
  document.querySelectorAll('h1, h2, h3.split').forEach(heading => {
    // Skip headings inside the loader
    if (heading.closest('#site-loader')) return;

    const split = new SplitType(heading, { types: 'words' });

    // Set starting state
    gsap.set(split.words, { opacity: 0, y: 55 });
