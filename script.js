...
═══════════════════════════════════════════════ */
/* Disable Lenis on touch devices — native momentum scrolling is smoother
   and avoids the jitter caused by JS and the browser fighting over scroll position,
   especially on Firefox mobile. */
const _isTouchDevice = window.matchMedia('(hover: none) and (pointer: coarse)').matches;

let lenis;
if (_isTouchDevice) {
  // Stub Lenis so code that references lenis.start()/stop() doesn't break
  lenis = { start(){}, stop(){}, scrollTo(){}, on(){} };
} else {
  lenis = new Lenis({
    duration: 1.25,
...
/* On mobile, allow some lag smoothing so GSAP doesn't stutter on dropped frames.
   On desktop, 0 keeps things crisp for parallax. */
if (_isTouchDevice) {
...
═══════════════════════════════════════════════ */
/* The loader shows the "DIJO STUDIOS" slot-machine scramble on a solid gold
   background. When all letters land, the gold cover retracts toward the
   bottom-left corner via an animated clip-path: circle() — same organic
   curved-edge reveal used for all page-to-page Barba transitions, so the
   whole site feels cohesive. */
function initLoader(onComplete) {
  // Pause the smooth scrolling system during the animation
  if (typeof lenis !== 'undefined') lenis.stop();

  // Create the visual elements on your page automatically.
  // Single full-screen gold layer — its clip-path animates to reveal the page.
    ...
    setTimeout(() => {
      const iv = setInterval(() => {
        if (frame < SPINS) {
          ...
          span.style.color = '#0a0a0a'; // Dark text on gold bg during scramble
        } else {
          clearInterval(iv);
          span.textContent = finalChar;
          span.style.color = ''; // Returns to base CSS color (#0a0a0a on gold)
          lettersLanded++;

          // When all letters finish scrambling, trigger the blob reveal
  ...
  /* Organic blob reveal — the gold cover shrinks toward the bottom-left
     corner via an animated clip-path: circle(). The curved edge of the
     circle gives the "soft, organic" feel matching the reference video.
     Origin point (0% 100%) = bottom-left corner of the viewport. */
  function triggerBlobReveal() {
    ...
    // Initial state: full cover (circle radius large enough to cover viewport
    // from bottom-left). 200vmax is always larger than the diagonal.
    ...
    // Animate the clip-path radius from 200vmax → 0%, retracting the gold
    // cover toward the bottom-left corner and revealing the page underneath.
    tl.fromTo(blob,
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let lastY = 0;
  if (_isTouchDevice) {
    // Native scroll listener for mobile — hide header on scroll down, show on scroll up
    // RAF-throttled so we don't set style.transform on every single scroll event,
    // which causes layout thrashing on Firefox mobile.
...
═══════════════════════════════════════════════ */
/* Uses the SAME organic clip-path: circle() reveal as the loader, so the
   entire site (initial load + every page-to-page navigation) shares one
   cohesive transition language. The gold overlay grows from the bottom-left
   corner to cover the screen (leave phase), then retracts back to the
   bottom-left to reveal the new page (enter phase). */
function initBarba() {
  ...
  // Initial state: invisible (clip-path collapsed to 0 at bottom-left).
  ...
  barba.init({
    ...
    transitions: [{
      ...
      async leave(data) {
        ...
        // Grow the gold blob from bottom-left until it covers the screen.
        // 200vmax is always larger than the viewport diagonal, so the
        // entire screen is gold at progress = 1.
        await gsap.fromTo(blob,
          { clipPath: 'circle(0% at 0% 100%)' },
          {
            clipPath: 'circle(200vmax at 0% 100%)',
            duration: _isTouchDevice ? 0.55 : 0.7,
            ease: 'cubic-bezier(0.85, 0, 0.15, 1)'
          }
        );
      ...
      async enter(data) {
        // New page sits hidden under the gold cover while it loads
        gsap.set(data.next.container, { opacity: 0 });
        lenis.scrollTo(0, { immediate: true });

        // Hold the cover long enough for the new page to render
        const holdTime = _isTouchDevice ? 0.2 : 0.3;
        await gsap.delayedCall(holdTime, () => {});

        // Make new page visible underneath before blob retracts
        gsap.set(data.next.container, { opacity: 1 });

        // Retract the gold blob back toward the bottom-left corner,
        // revealing the new page underneath with the same curved edge.
        await gsap.fromTo(blob,
          { clipPath: 'circle(200vmax at 0% 100%)' },
          {
            clipPath: 'circle(0% at 0% 100%)',
...
/* ═══════════════════════════════════════════════
   7a. SCROLL INDICATOR
═══════════════════════════════════════════════ */
/* The glowing "Scroll" button at the bottom of the hero. Clicking it
   smoothly scrolls to the gallery. It also fades out once the user has
   started scrolling past the hero so it doesn't linger on screen. */
function initScrollIndicator() {
  const indicator = document.querySelector('.scroll-indicator');
  if (!indicator) return;

  // Click → smooth scroll to the gallery section
  indicator.addEventListener('click', () => {
    const gallery = document.querySelector('.gallery-section') || document.getElementById('gallery');
    if (!gallery) return;
    if (_isTouchDevice) {
      // Native smooth scroll on mobile (Lenis is disabled there)
  ...
  // Hide the indicator (arrow + glow bar) the moment the user scrolls away
  // from the very top of the page. Show it again only when they return to
  // scrollY === 0. A 4px threshold absorbs tiny Safari/iOS scroll jitter
  // that would otherwise cause flicker on tap or pinch.
  ...
  const updateIndicatorVisibility = () => {
    ...
    requestAnimationFrame(() => {
      ...
      // Arrow: fade out via inline opacity (preserves the CSS pulse animation
      // for when it is visible).
      indicator.style.opacity = shouldHide ? '0' : '';
      indicator.style.pointerEvents = shouldHide ? 'none' : '';
      // Glow bar: toggle a body class so the CSS rule on
      // `body.scrolled-from-top .hero-centered::after` can hide it.
...
function initAnimations() {
  // On mobile, skip SplitType — it creates many DOM elements per heading/paragraph
  // which hurts Firefox mobile performance and causes layout jank.
  // Instead, use simple fade-in reveals that look great without the overhead.
  if (_isTouchDevice) {
  ...
  // Skip image parallax on mobile — extra ScrollTrigger instances + transform updates
  // on every scroll frame add up to significant jank on Firefox mobile.
  if (!_isTouchDevice) {
...
function initPortfolioEffects() {
  ...
  destroyMouseFollow();
  // On touch devices, skip parallax & mouse-follow — they cause jitter and
  // don't make sense without a hover cursor. Only snake reveal + lightbox work on mobile.
...
function updateCardImg(card) {
  ...
  // Use translate3d to force GPU compositing — avoids repaints on Firefox.
  // toFixed(4) avoids sub-pixel jitter from floating point drift.
...
function snakeReveal(cards) {
  ...
  /* ── Scroll-scrubbed scale-up reveal ──
     Inspired by the attached reference video: as the user scrolls
     down past the hero, the gallery images grow from small/scaled-down
     to full size, all together with a soft stagger tied to scroll position.
     On mobile, we use a simpler non-scrubbed fade+scale (scrub is too
     janky on touch + low-end devices). */

  if (_isTouchDevice) {
    // Mobile: simple staggered fade-in + scale, fire once when gallery enters viewport
    ...
    gsap.to(ordered, {
      ...
      onComplete: () => {
        // Clear will-change now that reveal is done to free GPU layers.
        ordered.forEach(c => { c.style.willChange = ''; });
      }
    });
  } else {
    // Desktop: scroll-scrubbed scale-up — images grow as the user scrolls.
    // Cards start at scale 0.55, opacity 0. As progress goes 0→1 over the
    // scroll distance, each card eases to scale 1, opacity 1. A small per-card
    // delay creates a subtle staggered "wave" feel without being slow.
    // NOTE: We write transform directly (not via gsap.set) to avoid GSAP's
    // matrix transform conflicting with our onUpdate string-writes.
    ...
    // Per-card stagger delay (fraction of total progress). 0.35 = last card
    // starts animating when scroll is 35% through, so the whole wave completes
    // before the user scrolls past the gallery top.
    ...
    ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        ...
        ordered.forEach((card, i) => {
          // Local progress for this card, with stagger
          const delay = (i / ordered.length) * STAGGER_RANGE;
          const lp = Math.max(0, Math.min(1, (p - delay) / (1 - STAGGER_RANGE)));
          // Ease the local progress for smoother motion (power2.inOut-ish)
          const eased = lp < 0.5
            ? 2 * lp * lp
            : 1 - Math.pow(-2 * lp + 2, 2) / 2;
          const scale = 0.55 + eased * 0.45;  // 0.55 → 1
          // Only set transform — opacity handled together to avoid extra writes
          card.style.transform = `translate3d(0,0,0) scale(${scale.toFixed(4)})`;
          card.style.opacity = eased.toFixed(4);
        });
      },
      onLeave: () => {
        // Final state — make sure all cards are fully visible & cleared.
        // Once-only trigger means scrolling back up won't reset them.
        ordered.forEach(card => {
...
/* ── Lightbox pinch‑to‑zoom (mobile + trackpad) ──
   Fixed version: properly captures the gesture on the image element
   (touch-action: none in CSS) so the browser doesn't fight JS for the
   2-finger touchmove. Adds one-finger pan after a pinch so the user
   can reposition the zoomed image. Clamps pan so the image can't be
   dragged fully off-screen. */
(function initLightboxPinchZoom() {
  let scale = 1, panX = 0, panY = 0;
  let lastDist = 0, lastCenterX = 0, lastCenterY = 0;
  let lastTouchX = 0, lastTouchY = 0;
  let isPinching = false;     // 2-finger pinch in progress
  let isPanning = false;      // 1-finger pan of a zoomed image
  ...
  /* Clamp pan so the zoomed image can't be dragged completely off-screen.
     Allows the image to move at most halfway out of the viewport on any side. */
  function clampPan(img) {
    if (scale <= 1) { panX = 0; panY = 0; return; }
    const rect = img.getBoundingClientRect();
    // Visible (untransformed) image size, before the scale transform.
    // rect already includes the current transform, so divide by scale to
    // recover the unscaled size — that's the size our pan/translate is
    // measured against (translate happens BEFORE scale in the transform
    // string, but in screen px after scale is applied).
    const w = rect.width / scale;
    const h = rect.height / scale;
    // How far the scaled image extends beyond its unscaled box on each side:
  ...
  // ── touchstart: detect 2-finger pinch OR 1-finger pan start ──
  // Attached to document but gesture is only honoured when it BEGAN on
  // the lightbox image. passive: true because we never preventDefault here.
  document.addEventListener('touchstart', (e) => {
    const lb = document.getElementById('lightbox');
    if (!lb || !lb.classList.contains('open')) return;

    if (e.touches.length === 2) {
      // 2 fingers → pinch (only if at least one finger is on the image)
      const onImg = [...e.touches].some(t =>
        document.elementFromPoint(t.clientX, t.clientY)?.closest('#lb-img, .lb-img-wrap')
      );
      if (!onImg) return;
      isPinching = true;
      isPanning = false;
      ...
      // 1 finger on a zoomed image → start panning
      const t = e.touches[0];
  ...
  // ── touchmove: apply pinch OR pan. Attached with passive:false so we
  //    can preventDefault and stop the browser from scrolling the
  //    lightbox container (#lightbox has overflow-y:auto on mobile). ──
  document.addEventListener('touchmove', (e) => {
    ...
    if (isPinching && e.touches.length >= 2) {
      ...
      if (scale > 1) {
        // Pan with the midpoint of the two fingers
      ...
      e.preventDefault();
      const t = e.touches[0];
      panX += t.clientX - lastTouchX;
      panY += t.clientY - lastTouchY;
      clampPan(img);
      applyTransform(img);
      lastTouchX = t.clientX;
      lastTouchY = t.clientY;
  ...
  // ── touchend: transition between pinch → pan, or fully release ──
  document.addEventListener('touchend', (e) => {
    if (e.touches.length === 0) {
      // All fingers lifted — end the gesture entirely
      isPinching = false;
      isPanning = false;
      if (scale <= 1) resetZoom();
    } else if (e.touches.length === 1 && isPinching) {
      // Went from 2 fingers → 1. Switch to one-finger pan so the user
      // can reposition the now-zoomed image. Don't reset zoom.
      isPinching = false;
      if (scale > 1) {
        isPanning = true;
  ...
  document.addEventListener('touchend', (e) => {
    ...
    const img = document.getElementById('lb-img');
    if (!img) return;
    // Ignore if this touchend was part of a pinch/pan gesture
    ...
    if (now - lastTap < 300 && e.changedTouches.length === 1) {
      if (scale > 1) {
        ...
        // Centre the tap point under the viewport centre after scaling.
        // transform-origin is 50% 50%, so the image scales around its own
        // centre; we then translate so the tapped point lands at centre.
        panX = (rect.width / 2 - tapX) * scale;
        panY = (rect.height / 2 - tapY) * scale;
        img.classList.add('zoomed');
        clampPan(img);
        applyTransform(img);
      }
...
function initStaticGallery() {
  ...
  setTimeout(() => {
    ...
    // Set initial hidden state immediately so cards don't flash visible before
    // the ScrollTrigger fires. Use direct DOM writes for consistency with
    // the snakeReveal function (which also writes directly).
    const initScale = _isTouchDevice ? 0.7 : 0.55;
    allCards.forEach(c => {
      c.style.transform = `translate3d(0,0,0) scale(${initScale})`;
      c.style.opacity = '0';
      // Add will-change hint for the duration of the reveal so the GPU
      // promotes each card to its own layer. Removed after reveal completes.
